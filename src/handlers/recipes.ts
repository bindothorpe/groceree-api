...
import { eq, and, sql, is, desc } from "drizzle-orm";
import {
  recipes,
  ingredients,
  instructions,
...
recipe.get("/favorites/:username", async (c) => {
  ...
  try {
    const favoriteRecipes = await db
      ...
      .innerJoin(
        users,
        and(eq(recipes.userId, users.id), eq(users.username, username))
      )
...
recipe.post("/:id/favorite", async (c) => {
  ...
  try {
    ...
    const favorite = await db
      ...
      .from(userFavorites)
      .where(
        and(eq(userFavorites.recipeId, id), eq(userFavorites.userId, user.id))
      )
      .get();
...
recipe.post("/:id/image", async (c) => {
  ...
  try {
    ...
    if (!allowedTypes.includes(image.type)) {
      throw new ApiError(
        400,
        "Invalid image type. Allowed types: JPG, PNG, GIF"
      );
    }
